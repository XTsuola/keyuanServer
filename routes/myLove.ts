...
export function myLove(router: Router) {
  router
    .get("/myLove/getCookList", verifyToken, async (ctx) => { // 获取菜谱列表
        ...
        const total = await queryCount(sql, "cook")
        const data = await queryAll(sql, "cook",parseInt(params.pageSize),parseInt(params.pageNo));
        ctx.response.body = {
          "code": 200,
          "rows": data,
          "total": total,
    ...
    }).post("/myLove/addCook", verifyToken, async (ctx) => { // 新增菜谱
      const params = await ctx.request.body({
        type: "json",
      }).value;
      const lastInfo = await findLast("cook");
      let id = 0;
      if (lastInfo.length) {
        id = lastInfo[0].id;
      }
      ...
      const data = await add(sql, "cook");
      ctx.response.body = {
        "code": 200,
        "rows": data,
        "msg": "新增成功",
      };
    }).post("/myLove/updateCook", verifyToken, async (ctx) => { // 修改菜谱
      const params = await ctx.request.body({
        type: "json",
      }).value;
    ...
    }).get("/myLove/deleteCook", verifyToken, async (ctx) => { // 删除菜谱
      const params = helpers.getQuery(ctx);
      const sql = { _id: new ObjectId(params._id) };
      const data = await deleteData(sql, "cook");
      ctx.response.body = {
        "code": 200,
        "rows": data,
        "msg": "删除成功",
